...
plt.plot(hours_ahead, predicted_load[0], marker='o', color='green', linewidth=2, label='Forecast')

z = np.polyfit(hours_ahead, predicted_load[0], 1)
p = np.poly1d(z)

plt.plot(hours_ahead, p(hours_ahead), "r--", linewidth=2, label='Trend Line')

plt.title(f'Load Forecast for next {FORECAST} Hours with Trend Line')
plt.xlabel('Hours Ahead')
plt.ylabel('Load (kWh)')
plt.xticks(hours_ahead)
plt.grid(True, linestyle=':', alpha=0.7) # เพิ่มเส้น Grid เพื่อให้ลากสายตาอ่านค่าได้ง่ายขึ้น
plt.legend()
plt.tight_layout()
plt.show()
